...
class Moderation(commands.Cog, name="moderation"):
    ...
    async def kick(
        ...
        if member.guild_permissions.administrator:
            embed = discord.Embed(
                description="User has administrator permissions.", color=RED_COLOR
            )
    ...
    async def nick(
        ...
        Default is None, which will reset the nickname.
        """
    ...
    async def warning_add(
        ...
        embed = discord.Embed(
            description=f"**{member}** was warned by \
        ...
        try:
            await member.send(
                f"You were warned by **{context.author}** in \
                    **{context.guild.name}**!\nReason: {reason}"
            )
    ...
    async def warning_remove(
        ...
        embed = discord.Embed(
            description=f"I've removed the warning **#{warn_id}** from \
                **{member}**!\nTotal warns for this user: {total}",
            color=GREEN_COLOR,
        )
        await context.send(embed=embed)
    ...
    async def warning_list(self, context: Context, user: discord.User):
        ...
        embed = discord.Embed(title=f"Warnings of {user}", color=GREEN_COLOR)
        description = ""
        ...
        if len(warnings_list) == 0:
            ...
            for warning in warnings_list:
                description += f"• Warned by <@{warning[2]}>: **{warning[3]}** \
                    (<t:{warning[4]}>) - Warn ID #{warning[5]}\n"
    ...
    async def purge(self, context: Context, amount: int) -> None:
        ...
        # Bit of a hacky way to make sure the bot responds to the
        # interaction and doens't get a "Unknown Interaction" response
        await context.send(
            "Deleting messages..."
        )
